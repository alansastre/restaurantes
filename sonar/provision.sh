#!/usr/bin/env bash
# Autoprovision de SonarQube Community LOCAL (Linux / macOS / WSL / Git Bash).
...
# Idempotente. Ejecutar desde la raiz del repo:  ./sonar/provision.sh
set -euo pipefail
...
# Resolver credenciales y quitar la contrasena por defecto
if valid admin; then
  ...
  echo "Contrasena de 'admin' establecida (ya no usa la de por defecto)."
  PASS="$NEWPASS"
elif valid "$NEWPASS"; then
  PASS="$NEWPASS"   # ya estaba provisionada en una ejecucion anterior
else
  echo "ERROR: no puedo autenticar como admin (ni 'admin/admin' ni 'admin/$NEWPASS')"; exit 1
fi
AUTH="admin:$PASS"

# POST tolerante (no aborta si el recurso ya existia)
...
# SonarQube copia las condiciones estrictas de 'Sonar way'. Las borramos todas y dejamos solo las nuestras.
...
post /api/projects/create               --data-urlencode project=restaurantes-testing --data-urlencode name=restaurantes-testing

curl -s -u "$AUTH" -X POST "$SONAR/api/user_tokens/revoke" --data-urlencode name=local-analysis >/dev/null || true
TOKEN=$(curl -s -u "$AUTH" -X POST "$SONAR/api/user_tokens/generate" --data-urlencode name=local-analysis | grep -oE '"token":"[^"]+"' | cut -d'"' -f4)
[ -n "$TOKEN" ] || { echo "ERROR: no se pudo generar el token"; exit 1; }
printf '%s' "$TOKEN" > sonar/token.txt

echo ""
echo "OK -> login admin / $NEWPASS | gate 'Curso Adecco' por defecto | proyecto 'restaurantes-testing' | token en sonar/token.txt"
echo "Siguiente paso:  ./sonar/analyze.sh"
